...
Z = np.array([[1,0], [0,-1]])
shapy = (4,3) ##to be changed if variational circuit changes
num_wires = 4 ##to be changed if variational circuit changes
...
def circ_creator(int1, int2, U_gates, Thets): 
    i=0
    j=0 
    numbers_had=0               
...
def H_Matrix_final_calc(U_gates, Thets, Hamil_array, Hamil_coeffs, entangle_gates):
    ...
    for i, j in it.combinations_with_replacement(range(n), 2): #it goes over all combinations 
...
def my_gen_solve(matrixA, matrixB, size):   ##self-written algorithm for solving the generalized eigenvalue equation
    ...
    eigvecs_B = np.matrix.transpose(eigvecs_B) 
    ...
    eigvecB_tilde = np.matmul(eigvecs_B, eigvalsB_mat_special) 
    A1 = np.matmul(matrixA, eigvecB_tilde)
    matrixA_tilde = np.matmul((np.matrix.transpose(eigvecB_tilde)), A1) 
...
def smallest_real_w_norm_optimiz_eigh(H_til, S_til):   
...
def finding_start_of_tail(array, k_array, tol): #no longer in use. 
    compare_val = array[-1]
