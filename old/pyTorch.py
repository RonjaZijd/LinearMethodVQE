...
params = np.random.normal(0, np.pi, (nr_qubits, nr_layers, 3)) #random numbers in the size of 3x2x3
params = Variable(torch.tensor(params), requires_grad=True)

def layer(params, j):
    for i in range(nr_qubits):
        qml.RX(params[i, j, 0], wires=i) #x-rotation matrix with random number
        qml.RY(params[i, j, 1], wires=i) #y-rotat etc...
        qml.RZ(params[i, j, 2], wires=i)

    qml.CNOT(wires=[0, 1]) #entangling the qubits with cnot gates
    qml.CNOT(wires=[0, 2])
    qml.CNOT(wires=[1, 2])
