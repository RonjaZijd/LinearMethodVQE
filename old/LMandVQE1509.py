...
for n in range(num_steps_lm):
    ...
    for k_ind, k in enumerate(regularizations): 
        ...
        #print("Condition number H: ", np.linalg.cond(H_tilde))
        try:
            ...
            #print("Update vector of eig: ", update)
            #print("Update vector of eigh: ", update_own)
            _eigval_diffs.append(np.square(np.vdot(update, update_own)))
    ...
    if plot_lm:
        ...
        #plt.scatter(regularizations, _eigval_diffs, label='inner product update vectors')
        plt.legend()
