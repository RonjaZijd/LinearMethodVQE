...
name_csv_file_execs = "HHSystem234ItsReg0.01ExecsFile"
#current_name = f"HHnseed121wIntrapolationReg{Regularization}"
...
for n in range(num_steps_lm):
    ...
    regularizations = np.linspace(0,  1, 100, endpoint=True) ##disabling the tail
    ...
    for k_ind, k in enumerate(regularizations): 
        ...
        try:
            ...
            update_h = LM.smallest_real_w_norm_optimiz_eigh(H_tilde, S_tilde)
            ...
            _energies.append(_E)
            _theta1s.append(_Thets[theta1A][theta1B])
            _theta2s.append(_Thets[theta2A][theta2B])
            _theta3s.append(_Thets[theta3A][theta3B])

            _Thetsh = LM.new_thetsy(update_h, Thets)
            _Eh = energy_jit(_Thetsh)
            _thetash.append(_Thetsh)
            _energiesh.append(_Eh)
            _theta1sh.append(_Thetsh[theta1A][theta1B])
            _theta2sh.append(_Thetsh[theta2A][theta2B])
            _theta3sh.append(_Thetsh[theta2A][theta2B])

            condition_numbers_H.append(np.linalg.cond(H_tilde))
        except:
            print("I didn't converggeee")
            n_not_converged +=1
    ...
    k_max = LM.finding_start_of_tail(_energies, regularizations, 0.001)
    iterations_lm.append(n+1)

    #using interpolation to choose the appropriate k value and using that to update:
    f2 = interp1d(regularizations, _energies, kind='cubic')
    f = interp1d(regularizations,  _energies)
    f3 = interp1d(regularizations,  _energies, kind='quadratic')
    knew = np.linspace(np.min(regularizations), np.max(regularizations), num=300, endpoint=True)
    ...
    if plot_lm:
        ...
        ax1.scatter(regularizations, _theta1s, label = 'Eig')
        ax1.scatter(regularizations, _theta1sh, label='Eigh')
        ax1.set(ylabel=f'Thet {theta1A},{theta1B}')
        ax1.legend()
        ax2.scatter(regularizations, _theta2s, label='Eig')
        ax2.scatter(regularizations, _theta2sh, label='Eigh')
        ax2.set(ylabel=f'Thet {theta2A},{theta2B}')
        ax2.legend()
        ax3.scatter(regularizations, _theta3s, label='Eig')
        ax3.scatter(regularizations, _theta3sh, label='Eigh')
        ax3.set(ylabel=f'Thet {theta3A},{theta3B}')
        ax3.legend()
        ax4.scatter(regularizations, _energies, label='Eig')
        ax4.scatter(regularizations, _energiesh, label='Eigh')
        ax4.set(ylabel='Energy')
        ax4.legend()
        #ax5.scatter(regularizations, condition_numbers_H)
        plt.xlabel('K-Value')
        plt.show()
        
        ######################## Plotting with Interpolation ######################################################
        # plt.plot(regularizations, _energies, 'o', knew, f(knew), '-', knew, f2(knew), '--', knew, f3(knew), '-.')
        # plt.legend(['data', 'linear', 'cubic', 'quadratic'], loc='best')
        # #plt.show()
        # plt.savefig(f'{current_name}andIteration{n}.png')
        # plt.show()

        ############ Plotting with condition number ################################################################
...
#plt.savefig(f"HHSeed1220Reg{Regularization}FinalResults.png")
plt.show()
