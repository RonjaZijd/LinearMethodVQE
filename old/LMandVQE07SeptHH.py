...
dev_lm2 = qml.device('default.qubit', wires=no_of_wires-1, shots=None )
...
for n in range(num_steps_lm):
    ...
    for k_ind, k in enumerate(regularizations): 
        ...
        try:
            ...
            _Thetsh = LM.new_thetsy(update_own, Thets)
...
t_2_lm = time.process_time()
Thets = Thets_start

# Initial point
energy = get_energy(dev_lm2)
energy_grad = qml.grad(energy)
energy_jit = get_energy(dev_lm2)
E_start = energy_jit(Thets)
#Initialize memory
iterations_lm2 = [0]
energies_lm2 = [E_start]
k_max = 1
...
for n in range(num_steps_lm):
    ...
    for k_ind, k in enumerate(regularizations): 
        ...
        try:
            #update = LM.smallest_real_w_norm_optimiz(H_tilde, S_tilde)
            update = LM.smallest_real_w_norm_optimiz_eigh(H_tilde, S_tilde)
            _Thets = LM.new_thetsy(update, Thets)
            #Energ_temp = LM.energy_calc(circuit, Hamilt_written_out, dev_lm, Thets_temp)
            _E = energy_jit(_Thets)
            _thetas.append(_Thets)
    ...
    arg_chosen = np.argmin(_energies)
    Thets = np.reshape(_thetas[arg_chosen], Thets.shape) ##choose the new theta's of the lowest energy
    energies_lm2.append(_energies[arg_chosen])###pick the lowest energy. 
    k_max = LM.finding_start_of_tail(_energies, regularizations, 0.001)
    iterations_lm2.append(n+1)
    ...
    if energies_lm2[n]<(-1.095): ###########################stop condition
        print("Terminating early wrt absolute value")
        break
...
all_energies = [energies_lm, energies_lm2, energies_scipy, energies_grad, energies_adam]
all_final_energies = [energies_lm[-1], energies_scipy[-1], energies_grad[-1], energies_adam[-1]]
all_iterations = [iterations_lm, iterations_lm2, iterations_scipy, iterations_grad, iterations_adam]
all_execution_counts = [
    ...
    dev_lm2.num_executions + lm_scaling(num_steps_lm)
]
labels = ["Linear Method Eig", "Linear Method Eigh", "BFGS", "Gradient Descent", "ADAM"]
start_times = [t_0_lm, t_2_lm, t_0_scipy, t_0_grad, t_0_adam]
end_times = [t_1_lm, t_3_lm, t_1_scipy, t_1_grad, t_1_adam]
