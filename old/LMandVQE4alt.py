...
def actual_optimization(S_tilde, H_tilde, Thets, circuit, Hamilt_written_outt, dev2, U_gates, U_gener, matrix_length, H_gates, H_coeffs, entangel_gates):
    ...
    for k in non_temp_k_ar:
        ...
        for n in range(50):
            ...
            S = S_alternative_way(U_gates, U_gener, Thets, matrix_length)
            energ = energy_calc(circuit, Hamilt_written_outt, dev2, Thets)
            S_tilde = S_tilde_matrix(S)
            H_tilde = H_tilde_matrix(H, energ, E_grad(Thets, Hamilt_written_outt, circuit, dev2), k)
            update = smallest_real_w_norm_optimiz(H_tilde, S_tilde)
            Thets = new_thetsy(update, Thets)
            energy_array = np.append(energy_array, energ)
            n_array = np.append(n_array, n)
            print(n)
        plt.plot(n_array, energy_array, label = f'{k}' )
    
    plt.title("Smallest normalized real w/regularization of 100 - 0.001")
