...
Goal 21 July: 
    ...
    we have: 
        -access to the array
        -if I automatically put the array into wires and then record which wire is associated with a certain i and j value
        -almost more important to from int1 and int2 determine if they're on the same wire
        -and if they're on the same wire, start the algorithm from only that wire 
...
@qml.qnode(dev)
def real_circ_h(int1, int2, mat_len, U_gates, Thets, inp_array, entangle_gates):
...
@qml.qnode(dev)
def imagin_circ_h(int1, int2, mat_len, U_gates, Thets, inp_array, entangle_gates):
    i, j = circ_creator(int1, int2, U_gates, Thets)
    up_to_un_circ(int2, i, j, mat_len, U_gates, Thets) 
    final_entangled_gates_circ(entangle_gates)
    c_notting_hamil(inp_array.numpy())
...
def total_ham_element(int1, int2, mat_len, U_gates, Thets, hamiltonian_array, Hamil_coefs, entangle_gates):
    Ham = 0
    HamC = 0
    i=0
    while i<len(hamiltonian_array):
        small_h_real = real_circ_h(int1, int2, mat_len, U_gates, Thets, hamiltonian_array[i], entangle_gates)
        small_h_imag = imagin_circ_h(int1, int2, mat_len, U_gates, Thets, hamiltonian_array[i], entangle_gates)
   ...
   #print(np.shape(H_matrix))
    ...
    while i<matrix_length:
        ...
        while j<matrix_length:
            if j>i or j==i:
                H, Hc = total_ham_element(i, j, matrix_length, U_gates, Thets, Hamil_array, Hamil_coeffs, entangle_gates)
                H_matrix[i][j] = H
...
def H_alternative_way(U_gates, Thets, Hamil_array, Hamil_coeffs, entangle_gates):
    ...
    for i in range(matrix_length):
        for n in range(matrix_length):
            if n==i:
                H, Hc = total_ham_element(i, n, matrix_length, U_gates, Thets, Hamil_array, Hamil_coeffs, entangle_gates)
                H_matrix[i][n] = H
            if n>i:
                H, Hc = total_ham_element(i, n, matrix_length, U_gates, Thets, Hamil_array, Hamil_coeffs, entangle_gates) 
                H_matrix[i][n] = 2*H
...
def smallest_real_w_norm_optimiz(S_til, H_til):
    ...
    print("These are the full eigenvalues: ")
    print(eigvals)
    print("They are thus real: ")
    print(is_real(eigvals))
    print()
    eigvec_wanted = eigvecs[np.argmin(np.real(eigvals))]
    #print("Eigenvalue:")
    #print(eigvals[np.argmin(np.real(eigvals))])
...
def actual_optimization(Thets, circuit, Hamilt_written_outt, dev2, U_gates, H_gates, H_coeffs, entangel_gates):
    energy_array = []
    n_array = []
    energy_old =0
    shakey = []
    ...
    for n in range(10): ##100 is atm the maximum value of iterations 
        H = H_Matrix_final_calc(U_gates, Thets, H_gates, H_coeffs, entangel_gates)
        S = S_Matrix_final_calc_newy(U_gates, Thets)
    ...
    plt.title("Smallest normalized real w/regularization of 100 - 0.001 w/ shaking")
...
actual_optimization(Thets, circuit, Hamilt_written_outt, dev2, U_gates, H_VQE_gates, H_VQE_coeffs, entangle_gates)

wir, pos = numb_to_wire(4, U_gates)
