...
def S_newy(int1, int2, U_gates, Thets):
    ...
    print(state)
    print("This is the weird _aux_op: ")
    print(_aux_op)
...
def total_ham_element(int1, int2, U_gates, Thets, inp_arrays, Hamil_coeffs, entangle_wires):
    ...
    def circ_h(int1, int2, U_gates, Thets, entangle_wires):
        ...
        [qml.CNOT(wires=[ent_wires[0]+1, ent_wires[1]+1]) for ent_wires in entangle_wires] #don't understand this CNOT?
        return qml.state()

    state = circ_h(int1, int2, U_gates, Thets, entangle_wires)
    H = 0.0+1j*0.0
    for term, coeff in zip(inp_arrays, Hamil_coeffs):
        _state = c_not_hamil_matrix(term, num_wires+1) @ state #i still understand this
        H += coeff * np.vdot(_state, _aux_op @ _state) #also don't see why this line iis needed
