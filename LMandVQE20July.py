...
def S_alternative_way2(U_gates, U_gener, Thets, matrix_lenth):
    S_matrix = np.zeros(shape=(matrix_length, matrix_length), dtype=np.complex128)
    print("Testtrigger!")
    for i in range(matrix_length):
        for n in range(matrix_length):
...
def smallest_real_w_norm_optimiz(S_til, H_til):
    #print("Trying to diagonalize the matrices: ")
    #print("This is S_til: ")
    #print(S_til)
    #print()
    #print("This is H_til: ")
...
def actual_optimization(Thets, circuit, Hamilt_written_outt, dev2, U_gates, U_gener, matrix_length, H_gates, H_coeffs, entangel_gates):
    energy_array = []
    n_array = []
    energy_old =0
    shakey = []
...
actual_optimization(Thets, circuit, Hamilt_written_outt, dev2, U_gates, U_gener, matrix_length, H_VQE_gates, H_VQE_coeffs, entangle_gates)
